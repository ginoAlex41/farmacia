...
if ($_POST['funcion'] == 'crear') {
  ...
  $foto = 'prod_default.png';
  $producto->crear($nombre, $concentracion, $obs, $precio, $laboratorio, $tipo, $presentacion, $foto);
}
if ($_POST['funcion'] == 'editar') {
  ...
  $tipo = $_POST['tipo'];
  $presentacion = $_POST['presentacion'];
  $producto->editar($id, $nombre, $concentracion, $obs, $precio, $laboratorio, $tipo, $presentacion);
}
...
if ($_POST['funcion'] == 'cambiar_foto') {
  ...
  if (($_FILES['photo']['type'] == 'image/jpeg') || ($_FILES['photo']['type'] == 'image/png') || ($_FILES['photo']['type'] == 'image/gif')) {
    ...
    if ($foto != '../img/prod/prod_default.png') {
      unlink($foto);
    }
...
if ($_POST['funcion'] == 'reporte_productosExcel') {
  ...
  foreach ($json  as $key => $producto) {
    ...
    $Sheet->setCellValue('D' . $celda, $producto['obs']);
...
if ($_POST['funcion'] == 'rellenar_productos') {
  ...
  foreach ($producto->objetos as $objeto) {
    ...
    $json[] = array(
      ...
      'nombre' => $objeto->id_producto . ' | ' . $objeto->nombre . ' | ' . $objeto->concentracion . ' | ' . $objeto->obs . ' | ' . $objeto->laboratorio . ' | ' . $objeto->presentacion
    );
  }
  $jsonstring = json_encode($json);
  echo $jsonstring;
