...
class Laboratorio{
    ...
    function crear($nombre,$foto){
        ...
        if(!empty($this->objetos)){
            ...
            $sql="INSERT INTO laboratorio(nombre,foto) values (:nombre,:foto);";
            $query = $this->acceso->prepare($sql);
            $query->execute(array(':nombre'=>$nombre,':foto'=>$foto));
            echo 'add';
        }
    }
    function buscar(){
    ...
    function cambiar_logo($id,$nombre){
        $sql="SELECT foto FROM laboratorio where id_laboratorio=:id";
        $query = $this->acceso->prepare($sql);
        $query->execute(array(':id'=>$id));
        $this->objetos = $query->fetchall();

            $sql="UPDATE laboratorio SET foto=:nombre where id_laboratorio=:id";
            $query=$this->acceso->prepare($sql);
