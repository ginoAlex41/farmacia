...
class Venta
{
  ...
  function buscar()
  {
    $sql = "SELECT id_venta,fecha,cliente,ci,total, CONCAT(usuario.nombre_us,' ',usuario.apellidos_us) as vendedor,id_cliente FROM venta join usuario on vendedor=id_usuario";
    $query = $this->acceso->prepare($sql);
    $query->execute();
    $this->objetos = $query->fetchall();
    return $this->objetos;
  ...
  {
    $sql = "SELECT id_venta,fecha,cliente,ci,total, CONCAT(usuario.nombre_us,' ',usuario.apellidos_us) as vendedor,id_cliente FROM venta join usuario on vendedor=id_usuario and id_venta=:id_venta";
    $query = $this->acceso->prepare($sql);
    $query->execute(array(':id_venta' => $id_venta));
    $this->objetos = $query->fetchall();
    return $this->objetos;
  ...
  {
    $sql = "SELECT nombre,concentracion,obs,SUM(cantidad) as total FROM `venta` JOIN venta_producto ON id_venta=venta_id_venta JOIN producto ON id_producto=producto_id_producto WHERE year(fecha)=year(curdate()) and month(fecha) = month(curdate()) group by producto_id_producto order by total desc limit 5";
    $query = $this->acceso->prepare($sql);
    $query->execute();
    $this->objetos = $query->fetchall();
    return $this->objetos;
